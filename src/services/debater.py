...
from src.services.debater_evidence import add_web_evidence
from src.services.debater_ipc import DebaterIpcMixin
from src.services.debater_prompts import (
    build_system_prompt as _build_generic_prompt,
)
from src.services.debater_prompts import (
    build_turn_header,
)
...
class Debater(DebaterIpcMixin, BaseAgent):
    ...
    async def get_argument(self, history: list[dict], round_num: int = 0) -> str:
        ...
        enriched_history, has_web_evidence, self.last_sources = await add_web_evidence(
            topic=self.topic,
            stance=self.stance,
            round_num=round_num,
            client=self.client,
